...
export class SyncManagerLevel implements SyncManager {
  ...
  private async getDwnEventLog(options: {
    ...
    if (syncDirection === 'pull') {
      ...
      const dwnResponse = await this.agent.dwnManager.processRequest({
        author         : did,
        target         : did,
        messageType    : 'EventsGet',
        messageOptions : { cursor }
      ...
      eventsReply = dwnResponse.reply as EventsGetReply;
    }
  ...
  private async getDwnMessage(
    ...
    const reply: MessagesGetReply = messagesGetResponse.reply as MessagesGetReply;

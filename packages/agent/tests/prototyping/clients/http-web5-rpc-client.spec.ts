...
describe('HttpWeb5RpcClient', () => {
  ...
  describe('sendDidRequest', () => {
    it('should throw if json rpc server responds with an error', async () => {
      ...
      const responsePromise = client.sendDidRequest(request);
      await expect(responsePromise).to.eventually.be.rejectedWith(`Error encountered while processing response from ${testDwnUrl}`);
    });
  ...
  describe('getServerInfo',() => {
    ...
    it('throws for an invalid response', async () => {
      const mockResponse = new Response(JSON.stringify({}), { status: 500 });
      sinon.stub(globalThis, 'fetch').resolves(mockResponse);
      const serverInfoPromise = client.getServerInfo(testDwnUrl);
      await expect(serverInfoPromise).to.eventually.be.rejectedWith('HTTP (500)');
    });

...
describe('Web5RpcClient', () => {
  ...
  describe('sendDidRequest', () => {
    ...
    it('should throw if transport client is not found', async () => {
      ...
      const responsePromise = httpOnlyClient.sendDidRequest(request);
      await expect(responsePromise).to.eventually.be.rejectedWith('no ws: transport client available');
  ...
  describe('sendDwnRequest', () => {
    ...
    it('should throw if transport client is not found', async () => {
      ...
      // request with ws
      const responsePromise = httpOnlyClient.sendDwnRequest({
        dwnUrl    : 'ws://127.0.0.1',
        targetDid : alice.did,
        message,
      });

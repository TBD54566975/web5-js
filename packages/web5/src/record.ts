...
export type RecordUpdateOptions = {
  ...
  messageTimestamp?: RecordsWriteDescriptor['messageTimestamp'];
...
export class Record implements RecordModel {
  ...
  get dateModified() { return this.messageTimestamp; }
  ...
  get data() {
    ...
    if (!this._encodedData && !this._readableStream) {
      ...
      this._readableStream = this._web5Agent.processDwnRequest({
        author         : this.author,
    ...
    // Explicitly cast _encodedData as a Blob since if non-null, it has been converted from string to Blob.
  ...
  async delete(): Promise<RecordsDeleteResponse> {
    ...
    const agentResponse = await this._web5Agent.processDwnRequest({
      author         : this.author,
      messageOptions : { recordId: this.id },
  ...
  async send(target: string): Promise<any> {
    ...
    const { reply: { status } } = await this._web5Agent.sendDwnRequest({
  ...
  async update(options: RecordUpdateOptions = {}) {
    ...
    let updateMessage = { ...this._descriptor, ...options } as Partial<RecordsWriteOptions>;
    ...
    if (200 <= status.code && status.code <= 299) {
      ...
      if (options.data !== undefined) {
        this._encodedData = dataBlob; // Clear `encodedData` in case it was previously set.
      }
    }
   ...
   * TODO: Document method.
   */
  private static verifyPermittedMutation(propertiesToMutate: string[], mutableDescriptorProperties: Set<string>) {

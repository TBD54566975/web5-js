    ...
    web5.session
      ...
      .then(authorizedSessions => {
        ...
        const connectToIdentityProvider = initiateConnectProtocol({
          connectRelay, dataStore, origin: web5.origin, permissionsRequests, web5
        });
...
const initiateConnectProtocol = ({ connectRelay, dataStore, origin, permissionsRequests, web5 }: {
  connectRelay: string,
  ...
  return async function () {
    ...
    /** TEMPORARY TEST Connect Provider */
    const providerConnect = new ConnectProtocol();
    ...
    const initiator = await connect.createInitiator({
      connectRelay        : connectRelay,
      origin              : origin,
      permissionsRequests : permissionsRequests
    });

    // Listen for 'checkpoint' events emitted by the Connect Protocol initiator.
    initiator.on('checkpoint', async ({ handshakeState, phase }) => {
      console.log('got checkpoint in web5');
      await dataStore.set('connectState', JSON.stringify({ handshakeState, phase}));
    });

    // Listen for 'connectLink' events emitted by the Connect Protocol initiator.
    initiator.on('connectLink', async ({ connectLink }) => {
      ...
      provider = await providerConnect.createProvider({ connectLink });
    ...
    // Listen for 'challenge' events emitted by the Connect Protocol initiator.
    initiator.on('challenge', async ({ validatePin }) => {
      web5.session.emit('authenticateIdentityProvider', { validatePin });
    });
  };
};

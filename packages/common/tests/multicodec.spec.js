...
describe('Multicodec', () => {
    ...
    describe('addPrefix()', () => {
        ...
        it('throws an error when code and name input data missing', () => {
            expect(() => Multicodec.addPrefix({ data: new Uint8Array(0) })).to.throw(Error, `Either 'name' or 'code' must be defined, but not both.`);
        });
        it('throws an error when both code and name specified', () => {
            expect(() => Multicodec.addPrefix({ code: 0x99999, name: 'non-existent', data: new Uint8Array(0) })).to.throw(Error, `Either 'name' or 'code' must be defined, but not both.`);
        });
        it('throws an error when codec not found', () => {
            expect(() => Multicodec.addPrefix({ code: 0x99999, data: new Uint8Array(0) })).to.throw(Error, 'Unsupported multicodec: 629145');
            expect(() => Multicodec.addPrefix({ name: 'non-existent', data: new Uint8Array(0) })).to.throw(Error, 'Unsupported multicodec: non-existent');
        });
    });
    ...
    describe('removePrefix()', () => {
        ...
        it('throws an error when codec not found', () => {
            ...
            expect(() => Multicodec.removePrefix({ prefixedData: dataWithPrefix })).to.throw(Error, 'Unsupported multicodec: 100');
        });
    });
});
//# sourceMappingURL=multicodec.spec.js.map